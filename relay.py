...
from .counter import emailfunction
...
class Relay(object):
    ...
    def _relay(self):
        ...
        def get_next_slice():
            ...
            msg_slice, msg_ids = msg_ids[:1], msg_ids[:1]
            return msg_slice
    ...
    def _relay_messages(self, message_ids):
        ...
        for response_part in msg_data:
            if isinstance(response_part, tuple):
                emailround = emailfunction()
                eml = email.message_from_string(response_part[1])
                res = self.smtp.sendmail(eml['from'], emailround, eml.as_string())
